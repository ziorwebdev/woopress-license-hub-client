...
class Update {
	...
	public function add_fetch_data( $transient ) {
		...
		if ( isset( $update_link->error ) || filter_var( $update_link, FILTER_VALIDATE_URL ) === false ) {
			$plugin->upgrade_notice                           = esc_html__( 'Automatic updates are currently disabled. Please ensure your license is valid and activated to enable updates.', 'woopress-license-hub-client' );
			$transient->response[ $this->plugin->get_base() ] = $plugin;
			return $transient;
		}
