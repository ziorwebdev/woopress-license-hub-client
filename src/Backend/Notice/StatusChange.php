...
class StatusChange {
	...
	public function display_status_change_notice() {
		...
		$transient_key = 'woopress_license_hub_license_status_change_' . md5( $this->plugin->get_file() );
		...
		if ( $notice['dismissible'] ) {
				...
				jQuery(document).ready(function($) {
					$(document).on('click', '.notice-<?php echo esc_js( $notice['type'] ); ?>.is-dismissible .notice-dismiss', function() {
						$.ajax({
							url: ajaxurl,
							data: {
								action: 'woopress_license_hub_dismiss_license_notice',
								transient: '<?php echo esc_js( $transient_key ); ?>',
								security: '<?php echo esc_js( wp_create_nonce( 'woopress_license_hub_dismiss_license_notice' ) ); ?>'
							}
						});
					});
				});
			...
			add_action( 'wp_ajax_woopress_license_hub_dismiss_license_notice', array( $this, 'dismiss_notice' ) );
		}
	}
	...
	public function dismiss_notice() {
		// Verify nonce
		check_ajax_referer( 'woopress_license_hub_dismiss_license_notice', 'security' );

		// Check if user has permission
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( __( 'You do not have permission to perform this action', 'woopress-license-hub-client' ) );
		}
